...
def scrape_job_description(job_url: str) -> dict:
    ...
    try:
        ...
        return {"success": False, "error": "Could not find job description on the page"}

    except requests.RequestException as e:
        logger.error(f"Request error for {job_url}: {str(e)}")
        return {"success": False, "error": f"Failed to fetch job posting: {str(e)}"}
    except Exception as e:
        logger.error(f"Unexpected error scraping {job_url}: {str(e)}")
        return {"success": False, "error": f"Error scraping job posting: {str(e)}"}
